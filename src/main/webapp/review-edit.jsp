...
<title>GameVerse Academy — Edit Review</title>
...
.main { margin-left: var(--sidebar-w); width: calc(100% - var(--sidebar-w)); min-height: 100vh; padding: 36px 40px; position: relative; z-index: 1; display: flex; flex-direction: column; align-items: center; }
.page-title { font-family: 'Orbitron', sans-serif; font-size: 1.4rem; font-weight: 700; background: linear-gradient(90deg, var(--blue-bright), var(--purple-bright)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; margin-bottom: 36px; align-self: flex-start; }

/* MOD INFO BADGE */
.mod-info { display: inline-flex; align-items: center; gap: 10px; padding: 12px 20px; border-radius: 12px; background: rgba(139,92,246,0.08); border: 1px solid var(--border); margin-bottom: 28px; width: 100%; max-width: 640px; }
.mod-info-label { font-size: 0.65rem; font-family: 'Orbitron'; text-transform: uppercase; letter-spacing: 2px; color: var(--dim); }
.mod-info-title { font-family: 'Rajdhani'; font-size: 1rem; font-weight: 700; color: var(--purple-bright); }
...
.form-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: linear-gradient(90deg, var(--blue), var(--purple), var(--pink)); }
...
.btn-submit { flex: 1; padding: 14px; border-radius: 10px; background: linear-gradient(135deg, var(--blue), var(--purple)); color: white; font-family: 'Rajdhani'; font-size: 1rem; font-weight: 700; border: none; cursor: pointer; transition: all 0.3s; box-shadow: 0 4px 20px rgba(59,130,246,0.3); letter-spacing: 1px; }
.btn-submit:hover { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(59,130,246,0.5); }
...
<main class="main">
    <div class="page-title">✏️ Edit Review</div>
    ...
    <div class="mod-info">
        ...
        <div>
            <div class="mod-info-label">Reviewing</div>
            <div class="mod-info-title"><%= review.getModTitle() != null ? review.getModTitle() : "Unknown Mod" %></div>
        </div>
    </div>

    <div class="form-card">
        <form action="<%= request.getContextPath() %>/ReviewEditController" method="post">
            ...
            <div class="form-group">
                ...
                <div class="star-rating">
                    <input type="radio" id="s5" name="rating" value="5" <%= review.getRating() == 5 ? "checked" : "" %>><label for="s5">★</label>
                    <input type="radio" id="s4" name="rating" value="4" <%= review.getRating() == 4 ? "checked" : "" %>><label for="s4">★</label>
                    <input type="radio" id="s3" name="rating" value="3" <%= review.getRating() == 3 ? "checked" : "" %>><label for="s3">★</label>
                    <input type="radio" id="s2" name="rating" value="2" <%= review.getRating() == 2 ? "checked" : "" %>><label for="s2">★</label>
                    <input type="radio" id="s1" name="rating" value="1" <%= review.getRating() == 1 ? "checked" : "" %>><label for="s1">★</label>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="comment" class="form-control"><%= review.getComment() != null ? review.getComment() : "" %></textarea>
            </div>

            <!-- ACTIONS -->
            <div class="form-actions">
                <a href="<%= request.getContextPath() %>/reviews" class="btn-cancel">✕ Cancel</a>
                <button type="submit" class="btn-submit">✏️ Save Changes</button>
            </div>
        </form>
    </div>
    <% } else { %>
    <p style="color:var(--dim); font-family:'Rajdhani';">Review not found.</p>
    <% } %>
</main>
</body>
</html>
